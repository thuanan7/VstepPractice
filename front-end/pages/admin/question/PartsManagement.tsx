...
import { partRequest } from '@/app/api'
import PartsTable from './components/PartsTable'
import { TextField, Button } from '@mui/material'
import { SectionType } from '@/features/exam/type'
...
const PartsManagement: React.FC<ManagePartsProps> = ({ parentId }) => {
  ...
  const fetchParts = async () => {
    const response = await partRequest.getParts(parentId)
  ...
  const handleFormSubmit = async () => {
    if (editingPart) {
      await partRequest.updatePart(editingPart.id, formData)
    } else {
      await partRequest.createPart(formData)
    }
    setFormData({
      id: null,
      title: '',
  ...
  const handleDelete = async (id: number) => {
    await partRequest.deletePart(id)
    fetchParts()
  }
