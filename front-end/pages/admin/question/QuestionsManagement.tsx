...
const QuestionsManagement = ({ sectionId }: { sectionId: number }) => {
  ...
  const fetchQuestions = async () => {
    const response = await questionRequest.getQuestions(sectionId)
    setQuestions(response.data)
  }

  // Fetch options for a selected question
  const fetchOptions = async (questionId: number) => {
    const response = await questionRequest.getOptions(questionId)
  ...
  const handleQuestionSubmit = async () => {
    if (editingQuestion) {
      await questionRequest.updateQuestion(editingQuestion.id, formData)
    } else {
      await questionRequest.createQuestion({ ...formData, sectionId })
    }
  ...
  const handleOptionSubmit = async () => {
    if (editingOption) {
      await questionRequest.updateOption(editingOption.id, optionData)
    } else {
      await questionRequest.createOption({
        ...optionData,
    ...
    <div>
      ...
      <div style={{ marginTop: '1rem' }}>
        ...
        <TextField
          ...
          onChange={(e) =>
            setFormData({ ...formData, point: parseInt(e.target.value, 10) })
          }
        ...
        <TextField
          ...
          onChange={(e) =>
            setFormData({ ...formData, orderNum: parseInt(e.target.value, 10) })
          }
          style={{ marginBottom: '1rem' }}
        />
        ...
        <div style={{ marginTop: '2rem' }}>
                    ...
                    <Button
                      onClick={() => {
                        ...
                        setOptionData({
                          ...option,
                          questionId: selectedQuestion?.id,
                          orderNum: parseInt(option.orderNum, 10),
                        })
                      }}
                    >
                      Edit
                    ...
                    <Button
                      ...
                      onClick={() =>
                        questionRequest
                          ...
                          .then(() => fetchOptions(selectedQuestion.id))
                      }
                    >
                      Delete
                    </Button>
          ...
          <div style={{ marginTop: '1rem' }}>
            ...
            <TextField
              ...
              onChange={(e) =>
                setOptionData({ ...optionData, content: e.target.value })
              }
              style={{ marginBottom: '1rem' }}
            />
